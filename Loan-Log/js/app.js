...
function buildScheduleClient(principal, annualRatePct, tenureMonths, startDate, emi) {
  const r = (annualRatePct / 100) / 12;
  ...
  for (let i = 1; i <= tenureMonths; i++) {
    ...
    let principalComp = +(emi - interest).toFixed(2);
    let emiThis = emi;

    if (i === tenureMonths) {
      principalComp = balance;
      emiThis = +(principalComp + interest).toFixed(2);
    }
...
function previewLoan() {
  ...
  if (!v.borrower_name || !v.principal || !v.tenure_months || !v.start_date || isNaN(v.interest_rate)) {
    alert('Please fill in all required fields.');
    return;
  }
  const emi = calcEmi(v.principal, v.interest_rate, v.tenure_months);
  const schedule = buildScheduleClient(v.principal, v.interest_rate, v.tenure_months, v.start_date, emi);
...
async function saveLoan(e) {
  ...
  try {
    ...
    document.getElementById('scheduleBody').innerHTML = '<tr><td colspan="6" class="text-center text-muted py-4">Fill the form and click Preview</td></tr>';
...
async function loadLoans() {
  ...
  try {
    ...
    if (!loans.length) {
      body.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">No loans yet. Create one in the Calculator tab.</td></tr>';
      return;
    }
...
async function deleteLoan(id) {
  if (!confirm('Delete this loan and all its records? This cannot be undone.')) return;
...
async function openLoan(id, name) {
  currentLoanId = id;
  document.getElementById('loanModalTitle').textContent = `Loan #${id} — ${name}`;
